...
# Make IUnitOfWork implement Application Ports
...
{
    new IGenericRepository<T> Repository<T>() where T : class;
    new Task<int> SaveChangesAsync(CancellationToken cancellationToken = default);
    new Task BeginTransactionAsync(CancellationToken cancellationToken = default);
    new Task CommitTransactionAsync(CancellationToken cancellationToken = default);
    new Task RollbackTransactionAsync(CancellationToken cancellationToken = default);
}
'@

# Make IGenericRepository implement Application Ports
...
{
    new Task<T?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default);
    new Task<IReadOnlyList<T>> GetAllAsync(CancellationToken cancellationToken = default);
    new Task<T?> GetBySpecificationAsync(ISpecification<T> spec, CancellationToken cancellationToken = default);
    new Task<IReadOnlyList<T>> GetListBySpecificationAsync(ISpecification<T> spec, CancellationToken cancellationToken = default);
    new Task<int> CountAsync(ISpecification<T> spec, CancellationToken cancellationToken = default);
    new Task<bool> AnyAsync(ISpecification<T> spec, CancellationToken cancellationToken = default);
    new Task<T> AddAsync(T entity, CancellationToken cancellationToken = default);
    new Task<T> UpdateAsync(T entity);
    new Task DeleteAsync(T entity);
    new Task DeleteRangeAsync(IEnumerable<T> entities);
    new IQueryable<T> GetQuery();
}
'@

# Make UnitOfWork implement both interfaces explicitly
...
# Make GenericRepository implement both interfaces
...
# Fix DependencyInjection to use Application Ports
...
public static class DependencyInjection
{
    public static IServiceCollection AddPersistenceLayer(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddScoped(typeof(Application.Ports.IGenericRepository<>), typeof(GenericRepository<>));
        services.AddScoped<IUnitOfWork, UnitOfWork>();
        services.AddScoped<Application.Ports.IUnitOfWork>(sp => sp.GetRequiredService<IUnitOfWork>());
...
Write-Host "Persistence ports alignment applied!"
